...
const Navbar = ({isDarkMode, setIsDarkMode}) => {
  ...
  return (
    <>
      ...
      <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll ? "bg-white/30 bg-opacity-50 backdrop-blur-lg shadow-sm dark:bg-black dark:shadow-white/20" : ""}`}>
        <a href="#top">
          <Image
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="block md:hidden ml-3 cursor-pointer" >
            <Image src={isDarkMode ? assets.menu_white : assets.menu_black} onClick={openMenu} alt="menu icon" className="w-6" />
          </button>
        </div>

        {/* -- ----- mobile menu ----- -- */ }
        <ul ref={sideMenuRef} className="flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 dark:bg-purple-950 dark:text-white" >
